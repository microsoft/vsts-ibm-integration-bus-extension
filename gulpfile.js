...
var devManifestOverride = {
    public: false,
    name: "IBM Integration Bus-Dev",
    id: "ibm-integration-bus-vsts-extension-dev",
    publisher: "yacaovsnc"
};
...
gulp.task('packageprod', ['installtaskdeps', 'cleanpackagefiles'], function (cb) {
    ...
    exec('node ./node_modules/tfx-cli/_build/app.js extension create --manifest-globs ibm-integration-bus-vsts-extension.json --override ' + toOverrideString(prodManifestOverride), function (err, stdout, stderr) {
        console.log(stdout);
        console.log(stderr);
        cb(err);
    });
...
gulp.task('packagetest', ['installtaskdeps', 'cleanpackagefiles'], function (cb) {
    ...
    exec('node ./node_modules/tfx-cli/_build/app.js extension create --manifest-globs ibm-integration-bus-vsts-extension.json --override ' + toOverrideString(devManifestOverride), function (err, stdout, stderr) {
        console.log(stdout);
        console.log(stderr);
        cb(err);
    });
